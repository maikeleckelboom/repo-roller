...
`)
    ...
    .option('-n, --name-template <template>', 'Filename template with placeholders ({repo}, {date}, {branch}, etc.)')
    .option('--path-separator <style>', 'Path separator style: dash, dot, plus, underscore')
    .option('-l, --label <text>', 'Custom label for filename')
    .option('--no-date', 'Omit date from filename')
    .option('--use-time', 'Include time in filename')
    .option('--prefix-date', 'Put date at start of filename')
    .option('--show-tokens', 'Include token count in filename')
